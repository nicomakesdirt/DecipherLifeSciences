...
Load the `dslabs` package and figure out what is in the `us_contagious_diseases` dataset. Create a data frame, call it avg, that has a column for year, and a rate column containing the cases of Measles per 10,000 people per year in the US. Because we start in 1928, exclude Alaska and Hawaii. Make sure to take into account the number of weeks reported each year. If a week was not reported, it should not be included in the calculation of the rate.
